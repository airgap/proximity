...
//
// agent none + per-stage agents on purpose: the build/test work runs in the
// lyku-ci-services docker agent, but the Deploy stage runs on the controller
// (built-in) because `ssh` needs getpwuid() and the host jenkins uid has no
// passwd entry inside the container ("No user exists for uid"). With agent none
// the docker agent releases its executor before the Deploy stage claims built-in.
//
// Host tokens in /etc/default/jenkins-doppler:
...
pipeline {
    ...
    stages {
        stage('Build & test') {
            ...
            stages {
                stage('Checkout') {
                    steps {
                        ...
                        // The Deploy stage runs on the controller; hand it the deploy dir.
            ...
            post {
                always { sh 'docker logout ghcr.io >/dev/null 2>&1 || true' }
            }
        }
